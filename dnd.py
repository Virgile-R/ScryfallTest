...
def generate_monster_block(*monster):
    monster_name = '-'.join(monster).lower()
    
    apiurl = "https://www.dnd5eapi.co/api/monsters/"+ monster_name
    ...
    output = f"./img/{monster_name}.png"
    if path.exists(f'./img/{monster_name}.png'):
        return output
    else: 
        ...
        with tag('p'):
            if data['damage_immunities']:
                for i in data['damage_immunities']:
                    text(f"{i}, ") 
            else:
                text("None")
        doc.asis("""</div> <!-- property line -->
